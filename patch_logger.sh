...
public class CoreLogger {
  ...
  public CoreLogger(Logger logger, ObjectMapper objectMapper, List<String> obfuscateFields) {
    this.logger = logger;
  ...
  public CoreLogger(Logger logger, ObjectMapper objectMapper) {
    this(logger, objectMapper, List.of());
  }

  public LogBuilder info(String format, Object... args) {
    return new LogBuilder(logger, LogLevel.INFO, format, args, objectMapper, obfuscateFields);
  }

  public LogBuilder error(String format, Object... args) {
    return new LogBuilder(logger, LogLevel.ERROR, format, args, objectMapper, obfuscateFields);
  }

  public LogBuilder warn(String format, Object... args) {
    return new LogBuilder(logger, LogLevel.WARN, format, args, objectMapper, obfuscateFields);
  }

  public LogBuilder debug(String format, Object... args) {
    return new LogBuilder(logger, LogLevel.DEBUG, format, args, objectMapper, obfuscateFields);
  }
    ...
    public LogBuilder with(Object payload) {
      if (payload != null) {
        try {
          ...
          if (map != null) {
            LogSanitizer.sanitizeMap(map, obfuscateFields);
            map.forEach(
                (k, v) -> {
                  if (v != null) {
