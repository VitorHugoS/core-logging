...
/**
 * Filtro que substitui o Aspecto (@Around). 
 * Captura toda a requisição, independentemente de erros de conversão no DispatcherServlet.
 */
public class CoreLoggingFilter extends OncePerRequestFilter implements Ordered {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            if (wrapPayload) {
                logPayload((ContentCachingRequestWrapper) requestToUse, (ContentCachingResponseWrapper) responseToUse);
    ...
    private void logPayload(ContentCachingRequestWrapper request, ContentCachingResponseWrapper response) {
        // Extrai o Payload. (Neste ponto da implementação, conectaremos a ofuscação LGPD via Regex/Jackson)
